...
class IFCSemanticAgentNoKG:
    ...
    def _initialize_components(self):
        ...
        try:
            ...
            self.logger.info("All agent components initialized successfully (without knowledge graph)")
    ...
    def _extract_semantics(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.working_memory['extracted_entities']['semantic_concepts'] = semantic_result.key_concepts
    ...
    def _reflect_on_results(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if current_confidence < 0.5:
            reflections.append("Low confidence in alignments - may need more context or different approach")
        if len(self.working_memory.get('alignments', [])) == 0:
            reflections.append("No alignments found - may need to adjust entity extraction parameters")
    ...
    def _should_stop(self, react_step: ReActStep) -> bool:
        ...
        if len(self.react_history) >= 3:
            ...
            if all(conf < 0.2 for conf in recent_confidences):
                ...
                alignments = self.working_memory.get('alignments', [])
                if not alignments or all(getattr(a, 'confidence_score', a.get('confidence_score', 0)) < 0.3 for a in alignments):
                    return True
    ...
    def _generate_final_answer(self) -> str:
        ...
        if alignments:
            ...
            # Show top alignments
            top_alignments = sorted(alignments, key=self._get_alignment_confidence, reverse=True)[:5]
            if top_alignments:
                answer_parts.append("\n🏆 Top Alignments:")
                for i, alignment in enumerate(top_alignments, 1):
                    ...
                    answer_parts.append(f"  {i}. {ifc_entity} ↔ {reg_entity} (confidence: {confidence:.2f})")
            ...
            if overall_confidence > 0.7:
                answer_parts.append("✅ Strong semantic correspondence found between IFC and regulatory terminology.")
            elif overall_confidence > 0.4:
                answer_parts.append("⚠️ Moderate semantic correspondence found. Additional context may improve accuracy.")
            else:
                answer_parts.append("❌ Limited semantic correspondence found. Consider providing more specific data.")
        else:
        ...
        return "\n".join(answer_parts) if answer_parts else "Unable to complete semantic alignment analysis with the provided data."
    ...
    def _generate_enhanced_recommendations(self) -> List[str]:
        ...
        if not alignments:
            recommendations.append("Provide more specific IFC data or regulatory text for better alignment")
        ...
        if overall_confidence < 0.3:
            ...
            recommendations.append("Moderate confidence - consider adding more detailed entity descriptions")
        ...
        if not extracted_entities or all(not entities for entities in extracted_entities.values()):
            ...
            recommendations.append("Ensure IFC and regulatory texts contain relevant building terminology")
        
        # Process-based recommendations
        if len(self.react_history) >= self.max_iterations - 1:
            recommendations.append("Analysis reached maximum iterations - consider increasing iteration limit")
            recommendations.append("Complex queries may benefit from breaking down into smaller parts")
    ...
    def _generate_recommendations(self, alignments: List[Dict[str, Any]]) -> List[str]:
        ...
        if not alignments:
            ...
            if avg_confidence < 0.5:
                recommendations.append("Consider adding more context to improve alignment confidence")
